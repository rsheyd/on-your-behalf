...
export function buildPrompt({ profile = "", formContext = "", page, fields }) {
  ...
  const sourceSections = [
    profile.trim() ? `SAVED PROFILE (user-provided facts):\n---\n${profile.trim()}\n---` : "",
    formContext.trim() ? `CONTEXT FOR THIS FORM (user-provided facts):\n---\n${formContext.trim()}\n---` : ""
  ].filter(Boolean).join("\n\n");

  return `You fill web forms using only the user-provided facts below.
